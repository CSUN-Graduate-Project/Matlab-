function [h_s,t_s,e_s] = text_vectors(in)
...
e_s = dec2bin(8.*fixdata(in.signal.chest.EDA(k2)),7);

fileID = fopen('stressed.txt','w');
...
kns = find(labels~=2&labels~=0);
k2ns = index(kns);
h_ns = dec2bin(8.*fixdata(hrate(kns)),11);
t_ns = dec2bin(8.*fixdata(in.signal.chest.Temp(k2ns)),9);
e_ns = dec2bin(8.*fixdata(in.signal.chest.EDA(k2ns)),7);

fileID = fopen('not_stressed.txt','w');
